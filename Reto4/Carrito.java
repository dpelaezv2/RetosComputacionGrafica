...
public class Carrito extends JPanel implements KeyListener {
    
    public static final int WIDTH = 1000;
    public static final int HEIGHT = 1000;
    public static Edge[] edges;
    public static String fileName = "Reto4/carrito.txt";
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if(tecla == KeyEvent.VK_W) {
            double xAmount = amount*Math.cos(Math.toRadians(angle));
            double yAmount = amount*Math.sin(Math.toRadians(angle));
            Transform("t", amount*Math.cos(Math.toRadians(angle)), amount*Math.sin(Math.toRadians(angle)));
            xPos += xAmount;
            yPos += yAmount;
